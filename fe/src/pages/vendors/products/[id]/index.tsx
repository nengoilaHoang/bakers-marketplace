...
const ProductDetailPage: NextPageWithLayout = () => {
  ...
  return (
    <div style={page}>
      ...
      <div style={card}>
        <div style={row}>
          ...
          <button
            style={button}
            onClick={() =>
              run(() => productApi.updateStock(product.id, Number(stockValue)))
            }
          >
            Cập nhật
          </button>
      ...
      <div style={card}>
        <div style={{ ...muted, marginBottom: 10 }}>Ngưỡng cảnh báo</div>

        {alerts.length === 0 && <p style={muted}>Chưa đặt ngưỡng nào.</p>}

        {alerts.map((a) => (
          <div key={a.id} style={{ ...row, marginBottom: 8 }}>
            <span style={{ ...chip, minWidth: 90 }}>{a.alertType}</span>
            <b>{a.threshold}</b>
            <button
              style={buttonDanger}
              onClick={() =>
                run(() => productApi.removeAlert(product.id, a.alertType))
              }
            >
              Xóa
            </button>
          ...
          {product.tags.map((t) => (
            <span key={t.id} style={chip}>
              ...
              <button
                style={buttonX}
                onClick={() =>
                  run(() => productApi.removeTag(product.id, t.name))
                }
              >
                ×
              </button>
      ...
      <div style={card}>
        ...
        {product.notes.map((n) => (
          ...
          >
            ...
            <button
              style={buttonDanger}
              onClick={() => run(() => productApi.removeNote(product.id, n.id))}
            >
              Xóa
            </button>
          </div>
          ...
          {product.collections.map((c) => (
            <span key={c.id} style={chip}>
              ...
              <button
                style={buttonX}
                onClick={() =>
                  run(() => productApi.removeFromCollection(product.id, c.id))
                }
              >
                ×
              </button>
      ...
      <div style={card}>
        {product.media.length === 0 ? (
          <p style={muted}>Chưa có ảnh — backend chưa có API upload.</p>
        ) : (
          product.media.map((m) => (
