...
export async function getRecipes({
	...
	const response = await request<RecipesApiResponse>(
		'/recipes',
		{
			method: 'GET',
			signal,
...
export async function getRecipeById(
	...
	const response = await request<{ data: RecipeDetail }>(
...
export async function getMyRecipes({
	signal,
}: { signal?: AbortSignal } = {}): Promise<Recipe[]> {
	const response = await request<{ data: Recipe[] }>('/recipes/mine', {
		method: 'GET',
		signal,
	});
...
export async function createRecipe(
	...
	const response = await request<{ data: RecipeDetail | null }>('/recipes', {
		method: 'POST',
...
export async function deleteRecipe(id: string): Promise<void> {
	await request<{ data: Recipe }>(`/recipes/${encodeURIComponent(id)}`, {
		method: 'DELETE',
	});
}
